...
const onSubmit = async () => {
  ...
  try {
    const res = await contactoService.crearContacto(data)
    if (res) {
...
<template>
  <Card class="rounded-2xl shadow-sm border-gray-100 bg-white">
    <CardContent class="p-8">
      ...
      <template v-else>
        <div v-if="errorMsg" class="mb-6 p-4 bg-red-50 border border-red-200 text-red-700 rounded-xl flex items-start gap-3">
        ...
        <form @submit.prevent="onSubmit" class="space-y-6">
          ...
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label class="block text-sm font-semibold text-text-main mb-1.5 items-center gap-1.5">
